...
const PhoneSurvey = () => {
  ...
  const handleAgreement = (e) => {
    ...
    if (name === "all") {
      ...
      setAgreements((prev) => {
        ...
        updated.all = updated.terms && updated.privacy && updated.marketing && updated.thirdParty;
        return updated;
      });
    }
  };
  ...
  return (
    <>
      <div className="w-full flex justify-center overflow-x-visible py-20 px-6" style={{ minHeight: adjustedHeight }}>
        <div
          ref={contentRef}
        ...
        >
          <div className="w-full text-center">
            <h2 className="text-[20px] font-[font-8] text-[#01A69F] mb-2">KT휴대폰은 바로 바로샵</h2>
            <h1 className="text-[33px] font-[font-5] font-bold mb-6">전국 최저가 요금 확인하기</h1>
            <p className="text-[19px] font-[font-4] mb-6">원하시는 휴대폰 상품을 선택해 주세요.</p>
          </div>
          ...
          <div className="w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div className="flex flex-col">
                ...
                <input name="name" value={form.name} onChange={handleInput} placeholder="이름" className="border p-3 font-[font-4]" />
              </div>
              <div className="flex flex-col">
                <label className="font-[font-4] text-[15px] mb-1">생년월일</label>
                <input name="birth" value={form.birth} onChange={handleInput} placeholder="생년월일 6자리를 입력해 주세요." className="border p-3 font-[font-4]" />
              </div>
            </div>
            <div className="flex flex-col mb-4">
              <label className="font-[font-4] text-[15px] mb-1">휴대폰 번호</label>
              <input name="phone" value={form.phone} onChange={handleInput} placeholder="010-0000-0000" className="w-full border p-3 font-[font-4]" />
            </div>
            <div className="flex flex-col mb-9">
              <label className="font-[font-4] text-[15px] mb-1">신청 희망 단말기</label>
              <input name="device" value={form.device} onChange={handleInput} placeholder="예) S25" className="w-full border p-3 font-[font-4]" />
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-8">
              {["신규가입", "번호이동", "기기변경"].map((type) => (
                <button key={type} type="button" onClick={() => handleSelect("joinType", type)} className={boxStyle(isActive("joinType", type))}>
                  {type}
                </button>
              ))}
            </div>

            <p className="font-[font-4] mb-2">단말기 결제방식을 선택해 주세요.</p>
            <div className="grid grid-cols-5 gap-2 mb-8 w-full">
              {["일시불", "24개월", "30개월", "36개월", "48개월"].map((period) => (
                <button
                  key={period}
                  type="button"
            ...
            <div className="flex flex-col gap-2 mb-6">
              ...
              >
                <span className={`text-[22px] font-bold ${form.discountType === "단말할인" ? "text-[#FD3941]" : "text-black"}`}>
                  단말할인
                </span>
              ...
              >
                <span className={`text-[22px] font-bold flex items-center ${form.discountType === "요금할인" ? "text-[#FD3941]" : "text-black"}`}>
                  요금할인
                  <span className="ml-1 bg-[#FD3941] text-white px-2 py-[2px] rounded-full text-[12px] font-[font-4]">추천</span>
                </span>
                <span className="text-[18px] text-black font-[font-4] whitespace-nowrap">
                  24개월간 매월 요금 25% 할인
                </span>
            ...
            <div className="mb-6">
              <label className="block font-[font-4] text-[26px] mb-2">기타 요청사항</label>
              <textarea name="additional" value={form.additional} onChange={handleInput} placeholder="추가로 상담 받고 싶은 내용이 있다면 적어주세요." className="w-full border p-3 text-[14px] font-[font-4]" rows={4} />
            </div>

            <div className="mb-6">
              <p className="font-[font-4] text-[14px] mb-2">약관을 확인해 주세요.</p>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <input type="checkbox" name="terms" checked={agreements.terms} onChange={handleAgreement} />
                  <span onClick={() => setIsPopupOpenInfo1(true)} className="text-black font-[font-4] text-[15px] underline cursor-pointer">서비스 이용약관 > <span className="text-[12px] font-[font-4]">(필수)</span></span>
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" name="privacy" checked={agreements.privacy} onChange={handleAgreement} />
                  <span onClick={() => setIsPopupOpenInfo2(true)} className="text-black font-[font-4] text-[15px] underline cursor-pointer">개인정보처리방침 > <span className="text-[12px] font-[font-4]">(필수)</span></span>
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" name="marketing" checked={agreements.marketing} onChange={handleAgreement} />
                  <span onClick={() => setIsPopupOpenInfo3(true)} className="text-black font-[font-4] text-[15px] underline cursor-pointer">마케팅 정보수신 동의 > <span className="text-[12px] font-[font-4]">(선택)</span></span>
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" name="thirdParty" checked={agreements.thirdParty} onChange={handleAgreement} />
                  <span onClick={() => setIsPopupOpenInfo4(true)} className="text-black font-[font-4] text-[15px] underline cursor-pointer">개인정보 수집 및 이용, 제3자 동의서 자세히 보기 > <span className="text-[12px] font-[font-4]">(선택)</span></span>
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" name="all" checked={agreements.all} onChange={handleAgreement} />
                  <span className="text-black font-[font-4] text-[16px]">전체 동의합니다.</span>
                </div>
              </div>
              {error && <p className="text-red-500 font-[font-4] text-[14px] mt-2">{error}</p>}
            </div>
        ...
        ReactDOM.createPortal(
          <>
            <div className="fixed inset-0 flex justify-center items-center z-[1000] px-4">
              <div className="w-full max-w-[819px] h-[100vh] bg-white rounded-[30px] border-4 border-red-500 overflow-y-auto relative flex flex-col">
                <div className="sticky top-0 bg-white p-6 border-b z-10">
                  <h2 className="text-[32px] font-bold text-[#000000] text-center">바로샵 비밀 지원금 문자 안내 <br />개인정보 수집 및 이용 동의서</h2>
                </div>
                <div className="flex-1 p-10 whitespace-pre-wrap text-[15px] leading-relaxed text-black">
                  {info4Text}
                </div>
