...
const InternetSurvey = () => {
  ...
  const handleAgreement = (e) => {
    ...
    if (name === "all") {
      setAgreements({ terms: checked, privacy: checked, marketing: checked, thirdParty: checked, all: checked });
    } else {
      setAgreements((prev) => {
  ...
  const buttonStyle = (active) => `flex-1 text-center border px-4 py-3 font-[Paperlogy] text-[18px] rounded-sm ${active ? "border-[#FD3941] text-[#FD3941] font-bold" : "border-black text-black"}`;
      ...
      <div className="w-full flex justify-center overflow-x-visible py-20 px-6" style={{ minHeight: adjustedHeight }}>
        ...
        >
          <div className="bg-white p-10 w-full text-center">
            <h2 className="text-[16px] font-[Paperlogy] font-bold mb-2">KT인터넷은 바로 바로샵</h2>
            <h1 className="text-[27px] font-[Paperlogy] font-bold mb-1">전국 최저가 요금 확인하기</h1>
            <p className="text-[14px] font-[Paperlogy] mb-6">원하시는 인터넷 상품을 선택해 주세요.</p>
          </div>

          <div className="bg-white pt-0 pb-10 w-full">
            <h3 className="text-[20px] font-[Paperlogy] mb-4">가입자 정보</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <input name="name" value={form.name} onChange={handleInput} placeholder="이름" className="border p-3 font-[Paperlogy]" />
              <input name="birth" value={form.birth} onChange={handleInput} placeholder="생년월일 6자리를 입력해 주세요." className="border p-3 font-[Paperlogy]" />
            </div>
            <input name="phone" value={form.phone} onChange={handleInput} placeholder="010-0000-0000" className="w-full border p-3 mb-4 font-[Paperlogy]" />

            <h3 className="text-[20px] font-[Paperlogy] mb-2">상담 기본정보</h3>
            <p className="font-[Paperlogy] text-red-600 text-[13px] mb-1 text-center">인터넷 단독 신청 시 사은품 전화상담 필수!</p>
            <p className="font-[Paperlogy] mb-2">가입유형을 선택해 주세요.</p>
            <div className="grid grid-cols-3 gap-2 mb-4">
              {joinTypes.map((type) => (
                <button key={type} type="button" onClick={() => handleSelect("joinType", type)} className={buttonStyle(isActive("joinType", type))}>{type}</button>
              ))}
            </div>

            <p className="font-[Paperlogy] mb-2">원하시는 사은품 종류를 선택해 주세요.</p>
            <div className="flex flex-col gap-2 mb-4">
              <button type="button" onClick={() => handleSelect("giftType", "상품권")} className={`border p-4 text-left font-[Paperlogy] text-[20px] ${isActive("giftType", "상품권") ? "border-[#FD3941] text-[#FD3941] font-bold" : "border-black text-black"}`}>
                <div className="flex justify-between items-center">
                  <span className="text-[20px] font-bold">상품권</span>
                  <span className="text-[14px] text-black font-normal">약 50만원 상당의 상품권 (지점별 상이)</span>
                </div>
              </button>

              <button type="button" onClick={() => handleSelect("giftType", "가전제품")} className={`border p-4 text-left font-[Paperlogy] text-[20px] ${isActive("giftType", "가전제품") ? "border-[#FD3941] text-[#FD3941] font-bold" : "border-black text-black"}`}>
                <div className="flex justify-between items-center">
                  <span className="flex items-center gap-2 font-bold">가전제품 <span className="text-[12px] bg-[#FD3941] text-white px-2 py-[2px] rounded-full">추천</span></span>
                  <span className="text-[14px] text-black font-normal">TV, 청소기, 공기청정기 등</span>
                </div>
              </button>
            </div>

            <div className="mb-6">
              <label className="block font-[Paperlogy] text-[16px] mb-2">기타 요청사항</label>
              <textarea name="additional" value={form.additional} onChange={handleInput} placeholder="추가로 상담 받고 싶은 내용이 있다면 적어주세요." className="w-full border p-3 text-[14px] font-[Paperlogy]" rows={4} />
            </div>

            <div className="mb-6">
              <p className="font-[Paperlogy] text-[14px] mb-2">약관을 확인해 주세요.</p>
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-black font-[Paperlogy] text-[16px]">전체 동의합니다.</span>
                </div>
              </div>
              {error && <p className="text-red-500 font-[Paperlogy] text-[14px] mt-2">{error}</p>}
            </div>

            <button onClick={handleSubmit} className="bg-[#00B7A3] text-white py-3 px-6 rounded-md font-[Paperlogy] text-[18px] w-full">
              상담 예약
            </button>
          </div>
        </div>
        ...
        ReactDOM.createPortal(
          <>
            <div className="fixed inset-0 flex justify-center items-center z-[1000] px-4">
              <div className="w-full max-w-[819px] h-[100vh] bg-white rounded-[30px] border-4 border-red-500 overflow-y-auto relative flex flex-col">
                <div className="sticky top-0 bg-white p-6 border-b z-10">
                  <h2 className="text-[32px] font-bold text-[#000000] text-center">개인정보 처리 W침</h2>
                </div>
                <div className="flex-1 p-10 whitespace-pre-wrap text-[15px] leading-relaxed text-black">
                  {info2Text}
                </div>
        ...
        ReactDOM.createPortal(
          <>
            <div className="fixed inset-0 flex justify-center items-center z-[1000] px-4">
              <div className="w-full max-w-[819px] h-[100vh] bg-white rounded-[30px] border-4 border-red-500 overflow-y-auto relative flex flex-col">
                <div className="sticky top-0 bg-white p-6 border-b z-10">
                  <h2 className="text-[32px] font-bold text-[#000000] text-center">개인정보 수집 및 이용 동의서</h2>
                </div>
                <div className="flex-1 p-10 whitespace-pre-wrap text-[15px] leading-relaxed text-black">
                  {info4Text}
                </div>
